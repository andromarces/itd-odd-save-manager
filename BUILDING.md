...
2.  **Install Frontend Dependencies**
    ...
    npm ci
    ```
    *Note: `npm ci` is recommended over `npm install` for reproducible environments as it respects `package-lock.json` strictly.*
...
*Note: Minor binary differences may occur due to timestamps or path variations, but are minimized via deterministic build flags in the CI.*
